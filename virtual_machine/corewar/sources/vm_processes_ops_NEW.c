...
void		load_process_NEW(t_area *area, int32_t player, uint32_t pc)
{
	t_process	*new;
...
void		new_process_NEW(t_area *area, t_process *process, uint32_t pc)
{
	t_process	*new;
...
int32_t			delete_not_live_processes_NEW(t_area *area)
{
	size_t		index;
	t_vector	*v;
	...
	{
		if (PRS_VALUE(v->v[index])->live_in_session == false)
		{
